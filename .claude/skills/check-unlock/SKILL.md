...
5. Record the finding by running `.claude/skills/check-unlock/scripts/record-unlock.sh <Program.exe> "<one-line description + ns.* hook finding>" "<code-change verdict>"` from the repo root — it appends a bullet in this exact format to the **"## Programs unlocked"** list in `/home/bosko/.claude/projects/-home-bosko-projects-bitburner/memory/bitburner_progression.md`: `- **<Program.exe>** (unlocked <today's date>). <description>. <verdict>`. Today's date is filled in automatically.
...
- `scripts/record-unlock.sh <Program.exe> "<description>" "<verdict>"` — step 5's memory write. Appends the formatted bullet immediately after the last existing `- **...` bullet under "## Programs unlocked" (kept as a separate script from `check-unlock.sh` so exactly one script in this skill ever writes to memory). Exits non-zero if the memory file or the expected bullet list isn't found, rather than guessing where to insert.
