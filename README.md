...
- **`karma.ts` upgraded from a one-shot print to a live HUD tail window**: shows current karma and karma/minute, following `battlestation.ts`'s existing tail-window pattern, refreshing every 5s. The rate is computed from elapsed time since the script started (not a per-poll delta), so the refresh interval and rate smoothness are independent.
